...
var checkDomain = function(auth) {
  ...
  log.notice("User rejected because " + email + " isn't in the domain list: " + requiredDomain.join(", "));
...
var checkEmail = function(auth) {
  ...
  log.notice("User rejected because " + email + "isn't in the list of allowed email addresses.");
...
exports.setup = function(everyauth) {
  everyauth.google
    ...
    .sendResponse( function (res, data) {
      var redirectTo = (data.session && data.session.redirectTo) || this.redirectPath();
      if (!redirectTo)
  ...
  if(requiredDomain) {
    log.notice("Registered Google authentication for members of domains: " + requiredDomain.join(", "));
  }
  if(requiredEmail) {
    log.notice("Registered Google authentication for users: " + requiredEmail.join(", "));
  }
};
